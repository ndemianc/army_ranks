...
{
	precache_sound("misc/multikill_v1.wav")
	precache_sound("misc/ultrakill_v1.wav")
	precache_sound("misc/killingspree_v1.wav")
	precache_sound("misc/rampage_v1.wav")
	precache_sound("misc/unstoppable_v1.wav")
	precache_sound("misc/monsterkill_v1.wav")
	precache_sound("misc/godlike_v1.wav")
}
...
public client_death(killer, victim, wpnindex, hitplace, TK)
{
	...
	if (MultiKill || MultiKillSound)
	{
		if (!selfkill && !TK && killer)
		{
			...
			set_task(3.0 + float(param[1]), "checkKills", 0, param, 2)
		}
	}
...
public checkKills(param[])
{
	...
	if (a == g_multiKills[id][0])
	{
		...
		if (a > -1)
		{
			...
			{
				new sound[32]
				format(sound, 31, "misc/%s_v1", g_Sounds[a]);
				log_amx("MISCSTATS.AMXX sound = %s", sound);
				play_sound(sound);
			}
		}
		g_multiKills[id] = {0, 0}
